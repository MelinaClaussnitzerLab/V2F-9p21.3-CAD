...
save_path_window = "/Volumes/broad_mcl/members_dir/bschmand/data/mac_seq/regression_output/smc/zscore_normalized/sliding_window"
save_path_snps = "/Volumes/broad_mcl/members_dir/bschmand/data/mac_seq/regression_output/smc/zscore_normalized/enhancer_and_snp_plots"
save_path_tss = "/Volumes/broad_mcl/members_dir/bschmand/data/mac_seq/regression_output/smc/zscore_normalized/tss"

export_screen = "/Volumes/broad_mcl/members_dir/bschmand/9p21_manuscript_figure_data/screen_fig"
...
test_data <- smc_merged %>% filter(GuidePosition < 22132000) %>%
...
dmrta1_enhancers <- smc_merged %>% filter(guide == "Neg-9p21 regions" | guide == "DMRTA1") ##%>% mutate(near = guide)
...
#test <- dmrta1_enhancers[5:722,1:35]
test <- dmrta1_enhancers[1:35]
test <- test[!duplicated(test$sequence),]
#write.csv(dmrta1_enhancers[5:722,1:35], paste0(save_path_snps,"/enhancer_effects_for_prism.csv"))
write.csv(test, paste0(save_path_snps,"/enhancer_effects_for_prism.csv"))

#repeat for mtap
mtap_enhancers <- smc_merged %>% filter(guide == "Neg-9p21 regions" | guide == "MTAP") ##%>% mutate(near = guide)
...
#for cdkn2a
cdkn2a_enhancers <- smc_merged %>% filter(guide == "Neg-9p21 regions" | guide == "CDKN2A") #%>% mutate(near = guide)
...
#for cdkn2b
cdkn2b_enhancers <- smc_merged %>% filter(guide == "Neg-9p21 regions" | guide == "CDKN2B") #%>% mutate(near = guide)
...
#for anril
anril_enhancers <- smc_merged %>% filter(guide == "Neg-9p21 regions" | guide == "CDKN2B-AS1") #%>% mutate(near = guide)
...
#for mir31hg
mir31hg_enhancers <- smc_merged %>% filter(guide == "Neg-9p21 regions" | guide == "MIR31HG") #%>% mutate(near = guide)
...
dmrta1_snps <- smc_merged %>% filter(guide == "Neg-9p21 regions") #%>% mutate(near = guide)
for (i in 1:nrow(snp_key)) {
  z <- snp_key[i,2]
  y <- smc_merged %>% filter((z-150)<GuidePosition & (z+150)>GuidePosition)
...
cdkn2a_snps <- smc_merged %>% filter(guide == "Neg-9p21 regions") #%>% mutate(near = guide)
for (i in 1:nrow(snp_key)) {
  z <- snp_key[i,2]
  y <- smc_merged %>% filter((z-150)<GuidePosition & (z+150)>GuidePosition)
  y <- y %>% mutate(near = snp_key[i,1])
...
anril_snps <- smc_merged %>% filter(guide == "Neg-9p21 regions") #%>% mutate(near = guide)
for (i in 1:nrow(snp_key)) {
  z <- snp_key[i,2]
  y <- smc_merged %>% filter((z-150)<GuidePosition & (z+150)>GuidePosition)
  y <- y %>% mutate(near = snp_key[i,1])
...
mtap_snps <- smc_merged %>% filter(guide == "Neg-9p21 regions") #%>% mutate(near = guide)
for (i in 1:nrow(snp_key)) {
  z <- snp_key[i,2]
  y <- smc_merged %>% filter((z-150)<GuidePosition & (z+150)>GuidePosition)
  y <- y %>% mutate(near = snp_key[i,1])
...
cdkn2b_snps <- smc_merged %>% filter(guide == "Neg-9p21 regions") #%>% mutate(near = guide)
for (i in 1:nrow(snp_key)) {
  z <- snp_key[i,2]
  y <- smc_merged %>% filter((z-150)<GuidePosition & (z+150)>GuidePosition)
  y <- y %>% mutate(near = snp_key[i,1])
...
mir31hg_snps <- smc_merged %>% filter(guide == "Neg-9p21 regions") #%>% mutate(near = guide)
for (i in 1:nrow(snp_key)) {
  z <- snp_key[i,2]
  y <- smc_merged %>% filter((z-150)<GuidePosition & (z+150)>GuidePosition)
  y <- y %>% mutate(near = snp_key[i,1])
...
tss_select <- smc_merged %>% filter(near %in% c("NT", "CDKN2B","CDKN2A","CDKN2B.AS1","DMRTA1","MTAP"))
...
ggsave(filename= file.path(export_fig4, "smc_tss_mtap.svg"), 
...
ggsave(filename= file.path(export_fig4, "smc_tss_dmrta1.svg"), 
       plot=tss_plot, width = 2.5, height = 5)
...
ggsave(filename= file.path(export_fig4, "smc_tss_cdkn2b.svg"), 
       plot=tss_plot, width = 2.5, height = 5)
...
ggsave(filename= file.path(export_fig4, "smc_tss_cdkn2a.svg"), 
       plot=tss_plot, width = 2.5, height = 5)
...
ggsave(filename= file.path(export_fig4, "smc_tss_anril.svg"), 
       plot=tss_plot, width = 2.5, height = 5)
