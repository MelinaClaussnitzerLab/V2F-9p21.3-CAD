...
save_path_snps = "/Volumes/broad_mcl/members_dir/bschmand/data/mac_seq/whole_screen_output/smc/zscore_normalized/enhancer_and_snp_plots"
save_path_tss = "/Volumes/broad_mcl/members_dir/bschmand/data/mac_seq/whole_screen_output/smc/zscore_normalized/tss"
...
# q <- 1
# for (i in windows) {
#   x <- smc_merged %>% filter((i-500)<GuidePosition & (i+500)>GuidePosition)
...
test <- c()
test <-  sliding_window_n(smc_merged, "CDKN2A.mean")

cdkn2b_window <-  sliding_window_n(smc_merged, "CDKN2B.mean")

cdkn2a_window <-  sliding_window_n(smc_merged, "CDKN2A.mean")

anril_window <-  sliding_window_n(smc_merged, "CDKN2B.AS1.mean")

dmrta1_window <-  sliding_window_n(smc_merged, "DMRTA1.mean")

mtap_window <-  sliding_window_n(smc_merged, "MTAP.mean")

mir31hg_window <-  sliding_window_n(smc_merged, "MIR31HG.mean")
...
dmrta1_plot <- nnfplot(dmrta1_plot,smc_merged, "DMRTA1.mean") #+ xlim(22064500,22135000) #xlim(22064500,22163000)
...
mtap_plot <- nnfplot(mtap_plot,smc_merged, "MTAP.mean") #+ xlim(22064500,22135000) #xlim(22064500,22163000)
...
mir31hg_plot <- nnfplot(mtap_plot,smc_merged, "MIR31HG.mean") #+ xlim(22064500,22135000) #xlim(22064500,22163000)
...
# smc_sample <- smc_merged %>% filter(guide_ctrl == "Screen")
...
#   #set.seed(q)
#   for (i in 1:164) {
#     temp[i,1] <- mean(sample(smc_sample$CDKN2B.mean, n_buckets[[i]]))
#   }
...
smc_sample <- smc_merged %>% filter(guide_ctrl == "Screen")
smc_sample_unique <- smc_sample[!duplicated(smc_sample$GuidePosition),]
#test_window <-  sliding_window_n(smc_sample_unique, "CDKN2B.mean")
for (q in 1:500) {
  x <- smc_sample_unique
  x$perm_val <- sample(smc_sample_unique$CDKN2B.mean, 536)
...
cdkn2b_plot <- simple_plot(cdkn2b_plot,smc_merged, "CDKN2B.mean")
...
#test_window <-  sliding_window_n(smc_sample_unique, "MTAP.mean")
for (q in 1:500) {
  x <- smc_sample_unique
  x$perm_val <- sample(smc_sample_unique$MTAP.mean, 536)
  temp <-  sliding_window_n(x, "perm_val")
  temp <- temp %>% mutate(perm_rep = paste0("V",q))
  #set.seed(q)
  #temp <- as.data.frame(t(temp))
...
mtap_plot <- simple_plot(mtap_plot,smc_merged, "MTAP.mean")
...
for (q in 1:500) {
  x <- smc_sample_unique
  x$perm_val <- sample(smc_sample_unique$CDKN2A.mean, 536)
  temp <-  sliding_window_n(x, "perm_val")
  temp <- temp %>% mutate(perm_rep = paste0("V",q))
  #set.seed(q)
  #temp <- as.data.frame(t(temp))
...
cdkn2a_plot <- simple_plot(cdkn2a_plot,smc_merged, "CDKN2A.mean")
y <- mtap_plot + ggtitle("MTAP") + loc_lims + guides(color="none") +
...
for (q in 1:500) {
  x <- smc_sample_unique
  x$perm_val <- sample(smc_sample_unique$CDKN2B.AS1.mean, 536)
  temp <-  sliding_window_n(x, "perm_val")
  temp <- temp %>% mutate(perm_rep = paste0("V",q))
  #set.seed(q)
  #temp <- as.data.frame(t(temp))
...
anril_plot <- simple_plot(anril_plot,smc_merged, "CDKN2B.AS1.mean")
...
for (q in 1:500) {
  x <- smc_sample_unique
  x$perm_val <- sample(smc_sample_unique$DMRTA1.mean, 536)
  temp <-  sliding_window_n(x, "perm_val")
  temp <- temp %>% mutate(perm_rep = paste0("V",q))
  #set.seed(q)
  #temp <- as.data.frame(t(temp))
...
dmrta1_plot <- simple_plot(dmrta1_plot,smc_merged, "DMRTA1.mean")
...
for (q in 1:500) {
  x <- smc_sample_unique
  x$perm_val <- sample(smc_sample_unique$MIR31HG.mean, 536)
  temp <-  sliding_window_n(x, "perm_val")
  temp <- temp %>% mutate(perm_rep = paste0("V",q))
  #set.seed(q)
  #temp <- as.data.frame(t(temp))
...
mir31hg_plot <- simple_plot(mir31hg_plot,smc_merged, "MIR31HG.mean")
...
####save all values####
write.csv(cdkn2b_permute, paste0(save_path_window, "/smc_cdkn2b_permute.csv"))
write.csv(mtap_permute, paste0(save_path_window, "/smc_mtap_permute.csv"))
write.csv(dmrta1_permute, paste0(save_path_window, "/smc_dmrta1_permute.csv"))
write.csv(cdkn2a_permute, paste0(save_path_window, "/smc_cdkn2a_permute.csv"))
write.csv(anril_permute, paste0(save_path_window, "/smc_cdkn2b_as1_permute.csv"))
write.csv(mir31hg_permute, paste0(save_path_window, "/smc_mir31hg_permute.csv"))

write.csv(cdkn2b_window, paste0(save_path_window, "/smc_cdkn2b_window.csv"))
write.csv(mtap_window, paste0(save_path_window, "/smc_mtap_window.csv"))
write.csv(dmrta1_window, paste0(save_path_window, "/smc_dmrta1_window.csv"))
write.csv(cdkn2a_window, paste0(save_path_window, "/smc_cdkn2a_window.csv"))
write.csv(anril_window, paste0(save_path_window, "/smc_cdkn2b_as1_window.csv"))
write.csv(mir31hg_window, paste0(save_path_window, "/smc_mir31hg_window.csv"))
...
smc_r1 <- smc_merged %>% filter(rep_id %% 2 == 0)
smc_r2 <- smc_merged %>% filter(rep_id %% 2 == 1)


window_reps <- sliding_window(smc_r1, "CDKN2B")
rownames(window_reps) <-  windows
temp <- sliding_window(smc_r2, "CDKN2B")
...
window_reps <- sliding_window(smc_r1, "CDKN2A")
rownames(window_reps) <-  windows
temp <- sliding_window(smc_r2, "CDKN2A")
rownames(temp) <- windows
window_reps <- data.frame(cbind(window_reps,temp))
colnames(window_reps) <- c("rep1","rep2")
...
window_reps <- sliding_window(smc_r1, "MTAP")
rownames(window_reps) <-  windows
temp <- sliding_window(smc_r2, "MTAP")
rownames(temp) <- windows
window_reps <- data.frame(cbind(window_reps,temp))
colnames(window_reps) <- c("rep1","rep2")
...
window_reps <- sliding_window(smc_r1, "CDKN2B.AS1")
rownames(window_reps) <-  windows
temp <- sliding_window(smc_r2, "CDKN2B.AS1")
rownames(temp) <- windows
window_reps <- data.frame(cbind(window_reps,temp))
colnames(window_reps) <- c("rep1","rep2")
...
window_reps <- sliding_window(smc_r1, "DMRTA1")
rownames(window_reps) <-  windows
temp <- sliding_window(smc_r2, "DMRTA1")
rownames(temp) <- windows
window_reps <- data.frame(cbind(window_reps,temp))
colnames(window_reps) <- c("rep1","rep2")
...
mir31hg_reps <- sliding_window(smc_r1, "MIR31HG")
rownames(window_reps) <-  windows
temp <- sliding_window(smc_r2, "MIR31HG")
rownames(temp) <- windows
window_reps <- data.frame(cbind(window_reps,temp))
colnames(window_reps) <- c("rep1","rep2")
